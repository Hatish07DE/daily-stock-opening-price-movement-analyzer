...
def run_pipeline(send_report: bool = True) -> None:
    ...
    Layer 2: Transformation
        - Calculate previous open vs current open movement
        - Clean and rank stock news

    Layer 3: Golden
        - Join price movement with news
    ...
    Observability:
        - Write pipeline run status JSON
    """
    ...
    try:
        ...
        logger.info("Processed news data saved to: %s", processed_news_file_path)
        logger.info("Processed news records created: %s", len(processed_news_df))
        ...
        logger.info("Golden summary records created: %s", len(golden_summary_df))
        ...
        success_payload = build_pipeline_status(
            ...
            processed_price_records=(
                len(price_movement_df) if price_movement_df is not None else 0
            ),
            processed_news_records=(
                len(processed_news_df) if processed_news_df is not None else 0
            ),
            golden_records=(
                len(golden_summary_df) if golden_summary_df is not None else 0
            ),
